...
class NirvanaCore {
  ...
  public static function defaultService() {
    NirvanaCore::$service['baseurl'] = function() {
      function baseurl($url = '') {
        return rtrim(NirvanaCore::$configure['baseurl'], '/') . '/' . ltrim($url, '/');
      }
    };
    ...
    NirvanaCore::$service['router'] = function() {
      function router($page, $strict=true) {
        if ($strict) {
          ...
          $current_route = str_replace(parse_url(baseurl(), PHP_URL_PATH), '', $_SERVER['REQUEST_URI']);
          $current_route = explode('?', $current_route)[0];
          $current_route = ltrim($current_route, '/');
          $current_route = preg_replace("/i=[12]/", "", $current_route);
    ...
    NirvanaCore::$service['is_similar_pattern'] = function() {
      function is_similar_pattern($pattern, $route) {
        // Split the pattern & route into arrays based on "/"
        ...
        // Loop through each segment and match
        foreach ($patternParts as $index => $part) {
          // If the segment is a dynamic parameter (e.g., {serial})
          if (preg_match('/^\{.+\}$/', $part)) {
            continue; // Consider it a match and proceed
          }
          // If the segment does not match exactly, return false
          if ($part !== $routeParts[$index]) {
            return false;
          }
        }
  ...
  public static function extractId($pattern, $value) {
    // Split pattern and value into arrays based on "/"
    $patternParts = explode('/', trim($pattern, '/'));
    $valueParts = explode('/', trim($value, '/'));

    // If the number of segments is different, they don't match
    if (count($patternParts) !== count($valueParts)) {
        return null;
    }

    // Initialize an array to store extracted parameters
    $params = [];

    foreach ($patternParts as $index => $part) {
        // Check if the segment contains a parameter placeholder (e.g., {param1})
        if (preg_match('/^\{(\w+)\}$/', $part, $match)) {
            $paramName = $match[1]; // Extract parameter name
            $params[$paramName] = $valueParts[$index]; // Assign corresponding value
        }
    }
  ...
  public static function sanitizeMethod($input) {
    if (is_string($input)) {
      ...
      // Mencegah XSS dengan mengonversi karakter berbahaya ke entitas HTML
      // $input = htmlspecialchars($input, ENT_QUOTES, 'UTF-8');
  
      // Menghapus karakter kontrol (ASCII < 32), tapi membiarkan simbol
...
class Nirvana {
   ...
   * @return void
   */
  public static function documentation( $asText = false ) {
    $html = '';
    $html .= '<h4 style="margin-bottom:0">Nirvana Documentation</h4>';
    $html .=  '<p style="margin-top:0">Version - '.NirvanaCore::$version.'</p>';
    $html .=  '<hr>';
